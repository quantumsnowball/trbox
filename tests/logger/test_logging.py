...
LOG_FUNCTIONS = (debug, info, warning, error, critical, exception)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Log("1", x=2.0, y=True).by("Test"), " Test : 1 x=2.0 y=True "),
        (
            Log("1", x=2.0, y=True).by("Test").tag("test"),
            " Test : 1 x=2.0 y=True #test ",
        ),
        (Log("1", x=2.0, y=True).sparse(), "\n1\nx=2.0\ny=True\n"),
        (
            Log("1", x=2.0, y=True).by("Test").tag("test").sparse(),
            "\nTest :\n1\nx=2.0\ny=True\n#test\n",
        ),
    ],
)
