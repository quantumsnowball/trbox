    ...
    def on_window(self: Strategy, _: OhlcvWindow):
        ...
        info(
            Log('trading',
                ...
                equity=self.trader.equity).by(self).tag(SYMBOL).sparse())

    Trader(strategy=Strategy(on_window=on_window),
           market=YahooOHLCV(
...
def test_account_cash(cash: float):
    ...
    def on_tick(self: Strategy, _):
        ...
        info(Log(cash=self.trader.cash).by(self).tag('initial', 'cash'))

    Trader(strategy=Strategy(on_tick=on_tick),
           market=DummyPrice(SYMBOL, delay=0),
           broker=PaperEX(SYMBOL, initial_fund=cash)).run()
